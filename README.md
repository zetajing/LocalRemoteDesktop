...
- **剪贴板同步** — 双向自动同步文本
- **访问码认证** — 默认使用 8 位可读访问码，可在托盘菜单中查看、复制、重新生成或关闭
...
- **控制端** — 右键托盘图标 → 连接远程电脑 → 输入 IP 地址；访问码开关必须与远程端一致
- **左键单击托盘图标**快速连接

访问码默认启用。托盘菜单会直接显示本机访问码，连接窗口中的访问码也会明文显示，方便人工查看和转发。关闭访问码后，任何能访问本机 8932 端口的设备都可以直接连接，请只在可信网络中使用。
